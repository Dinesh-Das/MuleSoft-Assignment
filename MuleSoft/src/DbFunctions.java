...
public class DbFunctions {
	void Retrive() {
		   try {
			   Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mulesoft", "root", "root");
	...
	void delete(String movieName) {
	  ...
	  void update(String movieName,String actorName,String directorName,String releaseDate)
	  {
		  try {
				Connection	 connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mulesoft", "root", "root");
				System.out.println("Connected With the database successfully");
		        PreparedStatement preparedStatement=connection.prepareStatement("update movies set actor=?,director=?,releasedate=? where name=?");
